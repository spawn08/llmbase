...
**Reading guide:** In production, **KV-cache reuse** makes the **forward** on \(\tau_{\mathrm{prefix}}\) **once per reuse**, and autograd flows **only through** layers that affect \(\tau_{\mathrm{new}}\) (depending on checkpointing choices).
