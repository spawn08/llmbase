...
!!! interview "FAANG-Level Questions"
    1. **Explain** why autoregressive Transformer inference without KV caching leads to **\(O(T^2)\)** aggregate attention work for length \(T\).
        *Answer:* Without caching, each new token \(t\) would recompute keys and values for the entire prefix of length \(t\) at every layer, so step \(t\) costs \(\Theta(t)\) attention-related work and \(\sum_{t=1}^{T} t = \Theta(T^2)\) total over generation. With KV caching you only append one new \((K_t,V_t)\) per layer and attend once per step in \(\Theta(t)\) time at step \(t\), so total work is \(\Theta(T^2)\) versus \(\Theta(T^3)\) for naive full-sequence recomputation—or the key point: caching removes the redundant **re**computation of past positions.
