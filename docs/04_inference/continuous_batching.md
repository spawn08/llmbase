...
!!! interview "FAANG-Level Questions"
    1. Explain why static batching inflates tail latency when request lengths are skewed, and how continuous batching changes the scheduling unit.
        *Answer:* In static batching every sequence waits for the slowest to finish (or pays padding to \(\max_i \ell_i\)), so a 10-token chat and a 2k-token completion in the same batch both tie to ~2000 decode steps—tail latency tracks \(\max\) length. Continuous (iteration-level) batching reschedules **each decode step**: finished sequences leave and new ones join, so short jobs complete in ~their own length while the GPU keeps high occupancy—scheduling unit becomes **per-iteration active set**, not one fixed batch for the whole job.
    2. Write the formula for logical token index to block id and offset; why does internal fragmentation appear at most \(B_{\text{block}}-1\) tokens per sequence?
        *Answer:* For 0-based token index \(t\), \(b=\lfloor t / B_{\text{block}}\rfloor\) and offset \(o = t \bmod B_{\text{block}}\). The last block for length \(L\) may be only partially filled—wasted slots within that block are at most \(B_{\text{block}}-1\) tokens (e.g. \(L=500\), \(B_{\text{block}}=16\) \(\Rightarrow\) 32 blocks, last block has 4 used, 12 slack).
    3. How does PagedAttention reduce **external** fragmentation compared to contiguous per-request buffers?
        *Answer:* Contiguous allocators leave unusable gaps between variable-size reservations as requests complete—free bytes exist but not in one slab for the next large request. Fixed-size **physical** blocks drawn from a global pool amortize holes across sequences: any free block fits the next allocation, similar to OS paging reducing external fragmentation versus malloc of arbitrary sizes.
    4. Contrast **TTFT** and **per-token latency**; which phase is more compute-bound vs memory-bound for typical decoder-only transformers?
        *Answer:* TTFT is dominated by **prefill** (processing the prompt, queue wait, possibly chunked attention) plus any scheduling delay—often compute-heavy large matmuls. Per-token latency (ITL) in decode is usually **memory-bandwidth** limited at small batch (weights + KV reads per token); raising batch improves arithmetic intensity until compute-bound. So: TTFT \(\approx\) prefill+queue; ITL \(\approx\) decode steady state.
    5. How would you implement **fairness** between a long prefill and many short chat messages?
        *Answer:* Use **chunked prefill** (e.g. 512–2048 tokens per chunk) interleaved with decode rounds so one 8k prompt cannot monopolize the GPU for tens of milliseconds straight. Optionally weighted fair queues: cap consecutive prefill chunks per request, or priority classes (interactive vs batch) with token budgets—trading slightly higher TTFT for the large job to protect p95 ITL for chats.
    6. What breaks naive **CUDA graph** capture when batch membership changes every iteration?
        *Answer:* CUDA graphs bake in static launch parameters and kernel addresses for a **fixed** graph of ops; when active batch size, sequence lengths, or pointer sets change every step, the captured graph no longer matches the workload. Runtimes use **piecewise** graphs (bucketed shapes), graph-safe memory pools, or replay only stable subgraphs—otherwise capture fails or silently wrong results occur.
    7. How does **tensor parallelism** interact with per-sequence KV block tables?
        *Answer:* TP shards weight matrices across GPUs; each rank holds a slice of \(K,V\) consistent with its attention head partition—block tables are still **per-sequence** but **distributed**: each GPU’s physical blocks store its shard of KV for the same logical tokens. All ranks must agree on sequence length and block indices for collectives; the page table maps logical blocks to **local** physical blocks per rank.
    8. Describe **prefix caching** vs **paged allocation**—which saves compute, which saves memory layout overhead?
        *Answer:* **Prefix caching** (radix/hashes) deduplicates **identical** prompt KV across requests—saves **prefill FLOPs** and avoids recomputing attention on shared system prompts. **Paged allocation** solves **fragmentation** and over-reservation of KV tensors—saves **bytes** and enables higher concurrent sequences; it does not by itself skip compute on unseen prefixes.
    9. What metrics would you put on a dashboard to detect KV pool exhaustion before OOM?
        *Answer:* Track **KV cache utilization %** (allocated blocks / pool capacity), **time waiting for KV allocation**, **admission reject** rate, **fragmentation** or largest free block histogram, and **per-GPU reserved vs live** bytes. Alert when utilization exceeds ~85–90% sustained or wait time grows—before the next large request triggers OOM.
    10. How does **Little’s law** relate offered load, queue depth, and latency for an LLM API?
        *Answer:* In steady state \(L = \lambda W\): average requests **in system** \(L\) equals arrival rate \(\lambda\) times average time in system \(W\). If offered tokens/sec approaches capacity, \(\lambda\) rises while service time is fixed \(\Rightarrow\) queue depth \(L\) and latency \(W\) grow linearly unless you add replicas or shed load—explains why p95 latency blows up before GPU util hits 100%.
