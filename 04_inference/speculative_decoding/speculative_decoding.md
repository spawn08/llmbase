...
!!! interview "FAANG-Level Questions"
    1. **What problem** does speculative decoding solve, and **why** is the target model often **memory-bandwidth bound** at batch size 1?
        *Answer:* Autoregressive decoding pays a full forward pass through billions of weights to emit **one** token; at \(B=1\) tensor cores are underutilized and HBM bandwidth to stream weights dominates (“memory-bound”). Speculative decoding amortizes that expensive target forward across **several** tokens per round by drafting cheap proposals and verifying them in one batched target pass, attacking wall-clock latency when verification is implemented with good KV reuse and kernel fusion.
    2. **Explain** the **draft/verify** pattern in **one** sentence suitable for a PM.
        *Answer:* A small, fast model proposes several next tokens in a row, and the large “real” model checks those proposals in one batched step—accepting the prefix that matches—so users get answers faster without changing **which** model ultimately decides quality when the algorithm is exact.
    3. **How** does **parallel** verification differ from **\(K\)** **serial** target forwards for **\(K\)** proposed tokens?
        *Answer:* One causal forward on a sequence of length \(n+K\) computes all next-token logits for positions covering the \(K\) draft tokens simultaneously (with proper masking), reusing prefix computation and KV—cost scales like **one** long forward, not \(K\) independent cold starts. \(K\) serial forwards would re-read weights and rebuild attention from scratch \(K\) times, losing the matmul fusion and KV reuse that make verification cheap relative to \(K\) separate steps.
    4. **What** is **\(\min(1, p/q)\)** trying to enforce in **stochastic** acceptance?
        *Answer:* It is Metropolis–Hastings–style acceptance: when the draft **over-proposes** a token relative to the target (\(q > p\)), you only accept with probability \(p/q\) so you do not oversample high-\(q\) events; when \(p \ge q\), acceptance probability is 1. Chained correctly with residual sampling on rejection, the **marginal** next-token distribution matches the target \(p\), preserving exact sampling equivalence.
    5. **Why** can a **weak** draft **slow** the system down versus **baseline** decoding?
        *Answer:* You always pay draft forward(s) **plus** target verification; if proposals disagree often (low acceptance \(\alpha\)), you accept few tokens per round and add overhead without amortizing the target pass—net tokens per second can fall below vanilla decoding. Weak drafts also trigger frequent KV rollback or short accepted prefixes, wasting memory traffic and kernel launches.
    6. **Contrast** **greedy** speculative decoding with **exact sampling** preservation—what is each **good** for?
        *Answer:* Greedy speculative accepts draft tokens while argmax matches and fixes the first mismatch with the target argmax—implementation is simpler and preserves **deterministic** greedy outputs for codegen or evals. Exact stochastic speculative sampling preserves the **full** target distribution for chat or safety filters tuned on \(p\), at the cost of more complex residual sampling and RNG plumbing.
    7. **How** does **Medusa** avoid a **second** full model—what is verified at inference time?
        *Answer:* Medusa adds shallow parallel prediction heads on the **same** backbone to propose multiple future tokens from existing hidden states—no separate draft transformer. At inference, the main LM still provides logits to **verify** head proposals (accept/reject), so quality is anchored in the full model’s distribution while avoiding a second parameter load.
    8. **Name** three **production** footguns: **numerical** parity, **tokenizer** mismatch, **KV** rollback after reject.
        *Answer:* **Numerical:** BF16/FP16 softmax and tie-breaking can diverge from FP32 reference, breaking bit-exact greedy parity with non-speculative baselines. **Tokenizer:** draft and target must share vocabulary and BPE merge rules—otherwise proposed token IDs map to different strings and acceptance tests are meaningless. **KV rollback:** partial acceptance requires discarding speculative tail states in the cache or recomputing suffixes; buggy rollback corrupts context or leaks tokens across requests.
    9. **Write** a **rough** expected-accepted-length expression under **independent** per-token acceptance \(\alpha\) and cap \(K\).
        *Answer:* Treating each of up to \(K\) positions as an independent geometric-like extension, a common pedagogical sum is \(\mathbb{E}[L] \approx \sum_{j=1}^{K} \alpha^{j} = \alpha(1-\alpha^{K})/(1-\alpha)\) for \(\alpha<1\)—e.g. \(\alpha=0.7\), \(K=8\) gives \(\mathbb{E}[L] \approx 1.96\) accepted leading tokens per round under this idealization (real chains are Markov-dependent).
    10. **Why** does **distillation** of \(q\) toward **\(p\)** on **realistic** prompts raise **acceptance** \(\alpha\)—often the **highest-ROI** knob before **custom** kernels?
        *Answer:* Acceptance grows when draft and target assign similar mass to the same next tokens on the **actual** deployment distribution (chat, code, tool JSON)—distillation aligns \(q\) to \(p_\theta\)’s local conditionals, raising per-step agreement from e.g. 0.5 to 0.8+ without changing hardware. Kernel work yields constant-factor speedups; raising \(\alpha\) multiplies effective tokens per target forward—the dominant lever until draft quality saturates.
